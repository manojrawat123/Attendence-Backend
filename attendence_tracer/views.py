...
def validateLocation(latitude, longitude):
    try:
        ...
        specified_latitude = radians(28.7027027027027)
        specified_longitude = radians(77.13583877332049)
        ...
        if distance > 100:
            return Response({"error" : "Location not matched"}, status=status.HTTP_400_BAD_REQUEST)
                ...
                if(checkin_date == date):
                    ...
                    if attendence_serializer.is_valid():
                        ...
                        try:
                            name = request.user.name
                            user_email = request.user.email
                            email_1 = EmailMessage(f"{name}", f" Check Out Successfully", 'simply2cloud@gmail.com',["positive.mind.123456789@gmail.com"])
